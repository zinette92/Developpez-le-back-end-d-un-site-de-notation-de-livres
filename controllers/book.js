...
exports.getBestRatedBooks = (req, res, next) => {
  ...
  Book.find().sort({averageRating: -1}).limit(3)
    .then((bestRatesBooks) => {res.status(200).json(bestRatesBooks)})
    .catch((error) => res.status(404).json({ error }));
};
    ...
    .then((book) => {
      if(book.userId != req.auth.userId) {
          ...
          .then(() => {
            ...
            console.log("images/" + previousImage);
            fs.unlink("images/" + previousImage , (err) => {
              if (err) {
                  console.log("Une erreur s'est produite lors de la suppression de l'image: " + err);
              } else {
